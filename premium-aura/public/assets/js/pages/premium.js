...
export async function mount(el, ctx) {
  ...
  <div class="upgrade-banner" style="margin-bottom:16px"><i class="fa-solid fa-crown crown"></i>
    ...
    <div class="small" style="opacity:.85">${l.premium ? `Valid until ${esc(new Date(l.premium.expires_at).toISOString().slice(0, 10))} · ⚡ ${num(l.hourly_limit)}/hour · ${num(l.daily_limit)}/day`
    : `Free plan · ${num(l.free_hourly_limit)}/hour · ${num(l.free_daily_limit)}/day. Upgrade for faster speed.`}</div></div></div>
  ...
  <div class="card"><div class="card-head"><h2>VIP Member Plans</h2></div>
    <div class="plan-grid" data-plans>${r.plans.map((p) => `
      <div class="plan-card ${p.is_featured ? 'featured' : ''} ${selected?.id === p.id ? 'selected' : ''}" data-plan="${p.id}" tabindex="0" role="button">
        ...
        <div class="speed-main"><i class="fa-solid fa-bolt"></i> ${num(p.hourly_limit || l.free_hourly_limit)}<small>/hour</small></div>
        <div class="speed-sub">${num(p.daily_limit || l.free_daily_limit)} numbers / day</div>
