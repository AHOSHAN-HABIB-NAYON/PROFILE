...
/** "Continue with Google" button + divider, shown only when the admin enabled it. */
function googleBlock(label) {
  if (state.site.google_login !== '1') return '';
  const next = params.get('next');
  const href = `/auth/google${next ? `?next=${encodeURIComponent(next)}` : ''}`;
  return `<a class="btn btn-google btn-block" href="${esc(href)}">${GOOGLE_G}<span>${esc(label)}</span></a>
...
const GOOGLE_ERRORS = {
  ...
  closed: 'New sign-ups are currently closed.',
  busy: 'Too many sign-in attempts. Please wait a few minutes.',
...
const views = {
  login() {
    ...
    const g = GOOGLE_ERRORS[params.get('google')];
    const notice = v === '1' ? alertBox('success', 'Email verified! You can now sign in.') : v === 'invalid' ? alertBox('error', 'That verification link is invalid or expired.') : g ? alertBox('error', g) : '';
