...
async function findResource(value, application = null) {
  ...
  if (exact || d.length < 8) return exact;
  // Same number in another format (e.g. "+964 78…" vs "078…"): one must be a suffix of the other.
  const rows = await db.query(
    `SELECT r.id, r.service_id, r.resource_value, r.serial_digits, r.assigned_user_id, r.status, s.country_code, s.country_name, s.app_code, s.app_name
     FROM authorized_resources r JOIN services s ON s.id = r.service_id
     WHERE r.serial_tail = ? AND r.status <> 'disabled' LIMIT 50`, [d.slice(-8)],
  );
  const hits = rows.filter((r) => r.serial_digits.length >= 8 && (d.endsWith(r.serial_digits) || r.serial_digits.endsWith(d)));
  hits.sort((a, b) => (b.app_code === application) - (a.app_code === application) || (b.status === 'assigned') - (a.status === 'assigned') || b.id - a.id);
  return hits[0] || null;
}
...
function publicEvent(row, { forAdmin = false, userId = null } = {}) {
  const e = {
...
async function ingest(provider, events, sourceId = null) {
  const stats = { inserted: 0, duplicates: 0, unauthorized: 0, unlisted: 0, credited: 0 };
  if (!events.length) return stats;
  const hours = await expirationHours();
  const publicFeed = await settings.getBool('otp_public_feed');
  const showCode = await settings.getBool('live_activity_show_code');
  const reward = money.normalize(await settings.get('event_reward') || '0.01');

  for (const ev of events) {
    const found = await findResource(ev.resource, ev.application);
    ...
    const result = await db.transaction(async (tx) => {
      const assignment = resource.id ? await tx.one(
...
async function feed(user, { page = 1, size = 30, app = null, q = null } = {}) {
  ...
  // Digits → end of the phone number ("last 4 digits"); anything else → part of the code.
  const numFilter = q && /^\d{3,15}$/.test(q) ? `%${q}` : null;
  const codeFilter = !numFilter && q && /^[A-Za-z0-9]{1,12}$/.test(q) ? `%${q}%` : null;
  const publicFeed = !isAdmin && await settings.getBool('otp_public_feed');

  const liveWhere = ["e.status = 'received'"];
  const liveParams = [];
  if (!isAdmin && !publicFeed) { liveWhere.push('e.user_id = ?'); liveParams.push(user.id); }
  ...
  if (numFilter) { liveWhere.push("REPLACE(REPLACE(e.resource_value, ' ', ''), '-', '') LIKE ?"); liveParams.push(numFilter); }
  ...
  if (numFilter) demoWhere.push('0 = 1');
  ...
  return {
    items: rows.map((r) => publicEvent(r, { forAdmin: isAdmin, userId: user.id })),
    public_feed: publicFeed || isAdmin,
    total,
